...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-[60] bg-black/70 backdrop-blur-xs transition-opacity duration-300 data-[state=closed]:opacity-0 data-[state=open]:opacity-100",
      className,
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  "fixed z-[70] gap-4 bg-background p-6 shadow-2xl transition-transform duration-300 ease-in-out",
  {
    variants: {
      side: {
        ...
        right:
          "inset-y-0 right-0 h-full w-4/5 max-w-sm border-l data-[state=closed]:translate-x-full data-[state=open]:translate-x-0",
      },
    },
    defaultVariants: {
      side: "right",
